/**
 * simply returns a function to decorate errors objects
 */
export type ErrorDecorator = (err: Error) => Error;

export function errorDecoratorFactory(funcName: string): ErrorDecorator {
  // creates a closure for returning formatted errors
  return (errorObj: Error) => {
    const decoratedError: Error = Object.create(errorObj);
    decoratedError.message = `${funcName}: ${errorObj.message}`;
